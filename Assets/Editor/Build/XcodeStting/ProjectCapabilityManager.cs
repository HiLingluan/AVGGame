...
#endif
{
    ...
    public class ProjectCapabilityManager
    {
        ...
        {
            UnityEngine.Debug.Log("WriteToFile: " + m_PBXProjectPath);
        ...
        {
            GetOrCreateEntitlementDoc().root[PushNotificationEntitlements.Key] =
                new PlistElementString(development ? PushNotificationEntitlements.DevelopmentValue : PushNotificationEntitlements.ProductionValue);
        ...
        {
            var arr =
                (GetOrCreateInfoDoc().root[GameCenterInfo.Key] ?? (GetOrCreateInfoDoc().root[GameCenterInfo.Key] = new PlistElementArray())) as
                PlistElementArray;
        ...
        public void AddSigin()
        {
            var array = new PlistElementArray();
            array.AddString("Default");
            GetOrCreateEntitlementDoc().root["com.apple.developer.applesignin"] = array;

            project.AddCapability(m_TargetGuid, PBXCapabilityType.Sigin);
        }
        ...
        {
            var bundleArr =
                (GetOrCreateInfoDoc().root[MapsInfo.BundleKey] ?? (GetOrCreateInfoDoc().root[MapsInfo.BundleKey] = new PlistElementArray())) as
                PlistElementArray;

...
namespace I2.Loc
{
    ...
    public class UpgradeManager
    {
        ...
        [MenuItem("Tools/I2 Localization/LocationPrefab")]
        public static void I2LocationPrefab()
        {
            GameObject[] objs = Selection.gameObjects;
            for (int i = 0; i < objs.Length; i++)
            {
                Debug.Log("预制体名称=====  " + objs[i].name);
                string rootName = objs[i].name.ToLower();
                TextMeshProUGUI[] textMeshs = objs[i].GetComponentsInChildren<TextMeshProUGUI>(true);
                for (int j = 0; j < textMeshs.Length; j++)
                {
                    string txtkey = rootName + "_" + j;
                    Debug.Log("需要替换多语言的组件名称 =====" + textMeshs[j].name);
                    if (textMeshs[j] != null)
                    {
                        Localize localize = textMeshs[j].gameObject.GetOrAddComponent<Localize>();
                        localize.SetTerm(txtkey);
